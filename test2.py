...
{Style.RESET_ALL}"""
    ...
    def packet_handler(self, packet):
        ...
        try:
            ...
            # Only process the packet in detail if advanced protection is enabled
            # This helps with performance
            if hasattr(self, 'advanced_protection') and self.advanced_protection:
                # If we have a thread pool, submit the packet for processing
                if hasattr(self, 'packet_executor') and self.packet_executor:
                    # Submit but don't wait - fire and forget to avoid blocking
                    try:
                        self.packet_executor.submit(self._process_packet, packet)
                    except Exception as submit_err:
                        logging.debug(f"Thread pool submission error: {str(submit_err)}")
                else:
                    # Process directly but only do minimal work
                    self._process_packet_minimal(packet)
            else:
                # For basic protection, just do IP validation if needed
                # Check only source IP for blacklisting
            ...
            logging.error(f"Error in packet_handler: {str(e)}")
    ...
    def _process_packet(self, packet):
        """Detailed packet processing logic"""
        try:
            # Perform actual packet analysis and threat detection here
            # This would include checking for SYN floods, port scans, etc.
            ...
            # We'll leave this minimal to ensure good performance
            pass
