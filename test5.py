...
{Style.RESET_ALL}"""
    ...
    def _process_packet(self, packet):
        """Detailed packet processing logic for threat detection"""
        try:
            current_time = time.time()
            
            # Check for IP in blacklist first (fast check)
            if hasattr(packet, 'src') and packet.haslayer('IP'):
                src_ip = packet.getlayer('IP').src
                if src_ip in getattr(self, 'blacklist', []):
                    with self.stats_lock:
            ...
            # ================ SYN FLOOD DETECTION ================
            # Check for TCP packets with SYN flag
            if packet.haslayer('TCP'):
                ...
                # Check if this is a SYN packet (only SYN flag set, no ACK)
                if tcp.flags == 0x02:  # SYN flag is 0x02
                    # Track SYN packets for SYN flood detection (per destination IP+port)
                    key = f"{dst_ip}:{dst_port}"
                    
                    with self.syn_lock:
                        # Create entry if it doesn't exist
                        if key not in self.syn_tracker:
                            self.syn_tracker[key] = {'count': 0, 'sources': {}, 'first_seen': current_time}
                        
                        # Increment counters
                        self.syn_tracker[key]['count'] += 1
                        
                        # Track each unique source
                        if src_ip not in self.syn_tracker[key]['sources']:
                            self.syn_tracker[key]['sources'][src_ip] = 0
                        self.syn_tracker[key]['sources'][src_ip] += 1
                        
                        # Check for SYN flood conditions
                        time_window = current_time - self.syn_tracker[key]['first_seen']
                        if (time_window <= self.syn_flood_timeout and 
                                self.syn_tracker[key]['count'] >= self.syn_flood_threshold):
                            
                            # Alert for SYN flood (limit alert frequency)
                            alert_key = f"synflood_{key}"
                            if (alert_key not in self.last_alert_time or 
                                    current_time - self.last_alert_time[alert_key] > self.flood_alerting_interval):
                                
                                # Log the detection
                                source_count = len(self.syn_tracker[key]['sources'])
                                logging.warning(
                                    f"SYN FLOOD DETECTED: {self.syn_tracker[key]['count']} SYN packets " +
                                    f"to {dst_ip}:{dst_port} from {source_count} sources in {time_window:.2f}s")
                                ...
                                with self.stats_lock:
                                    if 'syn_flood_detected' not in self.stats:
                                        self.stats['syn_flood_detected'] = 0
                                    self.stats['syn_flood_detected'] += 1
                                
                                # For severe attacks, consider blocking top sources
                                if self.syn_tracker[key]['count'] > self.syn_flood_threshold * 2:
                                    # Find the top offending sources
                                    top_sources = sorted(
                                        self.syn_tracker[key]['sources'].items(), 
                                        key=lambda x: x[1], 
                                        reverse=True
                                    )[:3]  # Top 3 sources
                                    ...
                                    for attack_src, count in top_sources:
                                        logging.warning(f"Blocking {attack_src} for SYN flood attack ({count} SYNs)")
                                        self.block_ip(attack_src, "SYN flood attack")
                
                # ================ PORT SCAN DETECTION ================
                # Track connections for port scanning detection
                # This detects when one source IP tries to connect to many ports on the same destination
                if tcp.flags & 0x02:  # Any packet with SYN flag set
                    with self.port_scan_lock:
                        ...
                        # Track each destination IP and port combination
                        if dst_ip not in self.port_scan_tracker[src_ip]['targets']:
                            self.port_scan_tracker[src_ip]['targets'][dst_ip] = set()
                        
                        # Add this port to the set of ports this source has tried
                        self.port_scan_tracker[src_ip]['targets'][dst_ip].add(dst_port)
                        
                        # Check for port scan conditions for each destination
                        time_window = current_time - self.port_scan_tracker[src_ip]['first_seen']
                        if time_window <= self.port_scan_timeout:
                            for target_ip, ports in self.port_scan_tracker[src_ip]['targets'].items():
                                if len(ports) >= self.port_scan_threshold:
                                    # Alert for port scan (limit alert frequency)
                                    alert_key = f"portscan_{src_ip}_{target_ip}"
                                    if (alert_key not in self.last_alert_time or 
                                            current_time - self.last_alert_time[alert_key] > self.flood_alerting_interval):
                                        
                                        # Log the detection
                                        logging.warning(
                                            f"PORT SCAN DETECTED: {src_ip} scanned {len(ports)} ports " +
                                            f"on {target_ip} in {time_window:.2f}s")
                                        
                                        # Increment detection statistics
                                        with self.stats_lock:
                                            if 'port_scan_detected' not in self.stats:
                                                self.stats['port_scan_detected'] = 0
                                            self.stats['port_scan_detected'] += 1
                                        
                                        # Update last alert time
                                        self.last_alert_time[alert_key] = current_time
                                        
                                        # Block the scanning IP
                                        logging.warning(f"Blocking {src_ip} for port scanning activity")
                                        self.block_ip(src_ip, "Port scanning activity")
            
            # Additional attack detection logic could be added here
            # - UDP flood detection
